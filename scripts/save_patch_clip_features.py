...
def parse_args():
    ...
    parser.add_argument("--clip_feat_path", default="/opt/tiger/activity_clip-14L_spatio_temporal_patch", help="The output dir to save the features in.")
    parser.add_argument("--infer_batch", required=False, type=int, default=16,
...
def main():
    ...
    vision_tower = CLIPVisionModel.from_pretrained('openai/clip-vit-large-patch14', torch_dtype=torch.float16,
                                                   low_cpu_mem_usage=True).to(rank)
    vision_tower.eval()
    ...
    for video_name in tqdm(all_videos):
        ...
        try:
            ...
            video_features = torch.FloatTensor(n_chunk, 257, 1024).fill_(0)
            ...
            for i in range(n_iter):
                ...
                # select_hidden_state = select_hidden_state[:, 1:]
                video_features[min_ind:max_ind] = select_hidden_state.detach().cpu()
            ...
            # video_clip_features[video_id] = video_features

        except Exception as e:
            print(f"Can't process {video_path}, error: {e}")

        # if counter % 32 == 0:  # Save after every 512 videos, update this number as per your requirements
