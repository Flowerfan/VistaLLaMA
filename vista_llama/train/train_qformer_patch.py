...
from vista_llama.train.llava_trainer import VistaLLaMAQformerTrainer
from vista_llama import video_conversation as conversation_lib
from vista_llama.model import *
...
class DataArguments:
    ...
    use_visual_mask: bool = False
    video_token_len: int = 128
...
def preprocess_multimodal(
    ...
    for source in sources:
        if multimodal_cfg["sep_video_conv_front"]:
            ...
            source[0]["value"] = (
                source[0]["value"].replace(DEFAULT_VIDEO_TOKEN, "").strip()
            )
            source[0]["value"] = (
                DEFAULT_VIDEO_TOKEN
        ...
        for sentence in source:
            ...
            if multimodal_cfg["use_vid_start_end"]:
                replace_token = (
                    DEFAULT_VID_START_TOKEN + replace_token + DEFAULT_VID_END_TOKEN
                )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.video_folder = self.multimodal_cfg["video_folder"]
        self.video_token_len = self.multimodal_cfg["video_token_len"]
        self.init_readers(self.video_folder)

    def init_readers(self, video_folder):
        data_names = [
            fname.split(".index")[0]
        ...
        readers = [
            KVReader(os.path.join(video_folder, data_name)) for data_name in data_names
        ]
        allkeys = [reader.list_keys() for reader in readers]
        key_to_reader = {}
        for num, keys in enumerate(allkeys):
            for key in keys:
                key_to_reader[key] = num

        self.readers = readers
        self.key_to_reader = key_to_reader

    def read_features(self, video_name):
        num_clip = self.video_token_len // 32
        reader = self.readers[self.key_to_reader[video_name]]
        feature = pickle.loads(reader.read_many([video_name])[0])
        split_inds = np.array_split(range(feature.shape[0]), num_clip)
        inds = [random.choice(inds) for inds in split_inds if len(inds) > 0]
        sample_feature = feature[inds]
        if len(sample_feature) < num_clip:
            pad_len = num_clip - len(sample_feature)
            pad_feature = sample_feature[-1:].repeat(pad_len, axis=0)
            sample_feature = np.concatenate([sample_feature, pad_feature], axis=0)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "video" in sources[0]:
            ...
            features = self.read_features(video_file.split(".")[0])

            cur_token_len = self.multimodal_cfg[
                "video_token_len"
            ]  # 100 temporal + 256 spatial, TODO: Hard Coding is not good
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if self.use_visual_mask:
            v_token = self.tokenizer.convert_tokens_to_ids([DEFAULT_VIDEO_PATCH_TOKEN])[
                0
            ]
            visual_mask = input_ids.eq(v_token)
        else:
            visual_mask = None
        batch = dict(
            input_ids=input_ids,
...
def make_supervised_data_module(
    ...
    data_collator = DataCollatorForSupervisedDataset(
        tokenizer=tokenizer, use_visual_mask=data_args.use_visual_mask
    )
    return dict(
...
def train():
    ...
    model.config.use_visual_mask = data_args.use_visual_mask
    model.config.qformer_type = model_args.qformer_type
    ...
    model.load_from_pretrained(
        "/mnt/bn/vlp-v6/VideoLLM/checkpoints/blip2_pretrained_flant5xxl.pth"
    )
    ...
    model.config.mm_use_vid_start_end = (
        data_args.mm_use_vid_start_end
    ) = model_args.mm_use_vid_start_end
    vision_config.use_vid_start_end = (
        training_args.use_vid_start_end
    ) = model_args.mm_use_vid_start_end
    model.config.sep_video_conv_front = data_args.sep_video_conv_front
    model.initialize_vision_tokenizer(
        mm_use_vid_start_end=model_args.mm_use_vid_start_end,
        tokenizer=tokenizer,
    ...
    trainer = VistaLLaMAQformerTrainer(
